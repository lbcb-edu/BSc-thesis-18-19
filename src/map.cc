...
//       parameters - mapping parameters
// Return: none
void process_pairs(std::vector<std::pair<mapping_t, mapping_t>>& mappings, paired_checked_t& checked,
  ...
  for (uint32_t j = 0; j < checked.first.size(); ++j) {
    ...
    std::string* quality2 = std::get<2>(region_pair.second.first)
                            ...
                            : &(second->quality);
    // std::pair<mapping_t, mapping_t> pair = pair_mapping(
    //     first->name,
    //     *query1, *quality1, 
    //     *query2, *quality2, 
