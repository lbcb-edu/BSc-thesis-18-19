...
namespace OSALG {
	...
	struct save1 {
		...
		save1(int m, int n, int p) : m(m), n(n), p(p)
		{}
	...
	void init_first_row(std::vector<alignment_info> &row) {
		...
		for (int i = 1; i < row.size(); ++i) {
			...
			for (int k = 0; k <= L + 1; ++k) {
				...
				row[i].p_arr[k] = (row[i].e_arr[k]) ? 0 : -1;
			}
		}
	}
	...
	void init_first_column_element(std::vector<alignment_info> &current_row, std::vector<alignment_info> &previous_row) {
		...
		for (int k = 0; k <= L + 1; ++k) {
			...
			current_row[0].p_arr[k] = (current_row[0].e_arr[k]) ? 0 : -1;
		}
	}

	void adr_function(int &pp, int m, int n, std::vector<alignment_info> &row, std::vector<SAVE1_type> &primary_list) {
		pp = primary_list.size();
	...
	void construct_CIGAR(std::vector<SAVE1_type> const &primary_list, std::string &cigar, std::string const &seq1, std::string const &seq2, bool extended_cigar) {
		...
		while(current_index > 0) {
			int next_index = primary_list[current_index].p;
